...
class _TambahRutinitasScreenState extends State<TambahRutinitasScreen> {
  ...
  Future<void> _simpan() async {
    ...
    try {
      final Map<String, dynamic> dataAktivitas = {
        "pasien_id": 1, 
      ...
      // DEBUG: Cek isi JSON di terminal
      debugPrint("Payload: ${jsonEncode(dataAktivitas)}");

      final response = await http.post(
        Uri.parse("http://10.0.2.2:8080/api/rutinitas"),
        headers: {"Content-Type": "application/json"},
        body: jsonEncode(dataAktivitas),
      );

      if (response.statusCode == 201 || response.statusCode == 200) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(backgroundColor: Colors.green, content: Text('Jadwal berhasil disimpan!')),
          );
          // Balik ke home sambil bawa sinyal 'true' buat refresh list
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Expanded(child: _buildTimeInput('Waktu Mulai', _waktuMulai, true)),
                          const SizedBox(width: 16),
                          Expanded(child: _buildTimeInput('Waktu Selesai', _waktuSelesai, false)),
                        ],
                      ),
                      const SizedBox(height: 24),
  ...
  Widget _buildNamaField() {
    return TextFormField(
      ...
      validator: (v) => (v == null || v.isEmpty) ? 'Kasih nama dulu dong' : null,
    );
  }
  ...
  Widget _buildTimeInput(String label, TimeOfDay time, bool isMulai) {
    return Column(
      ...
      children: [
        ...
        _buildTimePickerCard(value: _formatTime(time), onTap: () => _pickTime(isMulai: isMulai)),
      ],
    );
  }

  Widget _buildTimePickerCard({required String value, required VoidCallback onTap}) {
    return InkWell(
      onTap: onTap,
    ...
    return Wrap(
      ...
      children: _hariList.map((hari) {
        ...
        return GestureDetector(
          onTap: () => setState(() => isSelected ? _selectedHari.remove(hari) : _selectedHari.add(hari)),
          child: Container(
            width: 45,
            height: 45,
            decoration: BoxDecoration(
              ...
              boxShadow: isSelected ? [BoxShadow(color: _green.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 4))] : null,
            ),
            alignment: Alignment.center,
            child: Text(hari, style: TextStyle(color: isSelected ? Colors.white : _textSecondary, fontWeight: FontWeight.bold, fontSize: 12)),
          ),
        );
      }).toList(),
    );
    ...
    return Container(
      ...
      child: ElevatedButton(
        ...
        child: _isSaving 
          ...
          : const Text('Simpan Jadwal', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
      ),
    );
  }
}