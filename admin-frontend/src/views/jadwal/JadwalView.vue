<template>
  <LayoutWrapper>
    <div class="min-h-screen bg-gray-50">
      ...
      <div v-if="jadwalStore.currentStep > 0" class="bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h2 class="text-lg font-bold text-slate-900">Tambah Jadwal Obat</h2>
            <p class="text-xs text-gray-500">Isi formulir berikut untuk membuat jadwal minum obat pasien</p>
          </div>
        </div>
          ...
          <div v-for="(step, i) in jadwalStore.steps" :key="i" class="flex items-center gap-2">
            <div class="flex items-center gap-2">
              <div :class="[
                ...
                jadwalStore.currentStep === i + 1 ? 'bg-teal-600 text-white ring-4 ring-teal-100' :
                'bg-gray-200 text-gray-500'
              ]">
                <svg v-if="jadwalStore.currentStep > i + 1" class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div v-if="i < jadwalStore.steps.length - 1"
              :class="['w-12 h-0.5', jadwalStore.currentStep > i + 1 ? 'bg-teal-600' : 'bg-gray-200']">
            </div>
          </div>
        </div>
      </div>
